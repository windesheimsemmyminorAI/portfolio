...
> **📁 Pad-conventie (repo-structuur):** alle paden zijn relatief t.o.v. de repo-root. De beelden zijn tijdens de reorg (commit *"characters/wolfje + opschoning"*) ondergebracht onder
> `wolfje-media-agentic/phase-0-spec/characters/wolfje/`, hieronder afgekort als **`…/wolfje/`**.
> Let op: enkele semantisch onderscheiden groepen in deze catalogus (round-5-output, canon-kandidaat, rear-reference) staan in de repo fysiek samen in **`…/wolfje/renders/prompttest-abcd/`**.
...
|---|---|---|---|
| `niece_style_reference.jpeg` | `…/wolfje/style-reference/` | Gedetailleerde inkttekening; dient als stijlbasis | referentie |

### Ronde 2 — met echte foto's (9 beelden) → `…/wolfje/renders/round-2/`
...
### Ronde 3 — hero-foto + identity-block (6 beelden) → `…/wolfje/renders/round-3/`
...
### Bronfoto's — echte Wolfje (12 beelden) → `…/wolfje/input-photos/`

Gedeelde referentieset die in deze chat is gebruikt voor het verzamelen van de karakter-referentie. Klein genoeg voor git; conform `…/wolfje/input-photos/` (en `wolfje-media-agentic/reference-material/README.md`) mogen lichte referentieframes in de repo.
...
| `wolfje_voor.jpg` | — (vooraanzicht; byte-identiek aan `20260501_140724.jpg`) |

---

## Chat 02 — jongvolwassen-verfijning · Ronde 5 (6 juni 2026)

### Ronde 5 — geanalyseerde output → `…/wolfje/renders/prompttest-abcd/`
...
*Referentie: round 5 gebruikte een 4-foto-subset uit `…/wolfje/input-photos/` (front + profiel-closeup + lichaamsfoto + jongere vergelijkingsfoto), in het oorspronkelijke round-5-pakket hernoemd naar `wolfje-real-ref-01..04`. De originelen staan al in `…/wolfje/input-photos/`; geen nieuw beeld nodig.*

*De 4 test-prompts (A/B/C/D) en v3 staan in `…/wolfje/prompts/wolfje-jongvolwassen-analyse-en-4-prompts.md` en `…/wolfje/prompts/wolfje-jongvolwassen-prompt-v3.md`. Eventuele test-renders daarvan: zie missing-checklist.*

## Chat 03 — Gemini→Gemini + canon-lock (6 juni 2026)

### Cel-shaded canon-grade render → `…/wolfje/renders/prompttest-abcd/`
...
### Echte referentiefoto's (achter/front) → `…/wolfje/renders/prompttest-abcd/`
...
> **Vergrendelde canon & turnaround (staan al in de repo):** de officiële `wolfje-jongvolwassen-CANON.png` staat in `…/wolfje/canon/`, en de vergrendelde turnaround-cellen 01–05 (`wolfje-jv-turn-01..05-*.png`) in `…/wolfje/model-sheets/character-sheets/`. De v1/v2- en Render-kandidaten staan onder `…/wolfje/renders/` (o.a. `round-1/`, `canon-neck-edit/`, `rejected/`). Controleer de missing-checklist alleen nog voor wat daar ontbreekt.

## Chat 04 — turnaround front cel + push (7 juni 2026)

De beelden van deze chat zijn in de sessie zelf naar GitHub gepusht (via GitHub Desktop) en staan in de repo:

| Bestand | Wat | Waar in de repo |
|---|---|---|
| `wolfje-jv-turn-01-front.png` | Vergrendelde vooraanzicht-cel (de laatste, moeilijkste cel) | `…/wolfje/model-sheets/character-sheets/` |
| cel 01-kandidaten | Portfolio-bewijs van de front-cel-selectie | `…/wolfje/renders/rejected/turnaround-front/` |
| turnaround-strip (5 cellen) | Samengestelde complete turnaround | controleren in `…/wolfje/model-sheets/character-sheets/` |

Zie missing-checklist voor eventueel ontbrekende afgeleiden. De technische beeldverwerking van deze chat staat in `docs/tool-knowledge/image-processing-pipeline.md`.
